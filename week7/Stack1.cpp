...
int IsEmptyStack(int nTop) {    // 스택이 비었는지에 따라 T/F를 반환한다.
//    if (nTop == -1)
//        return true;
//    return false;
    return nTop == -1;
...
int IsFullStack(int nTop) {    // 스택이 가득 찼는지에 따라 T/F를 반환한다.
    return nTop == N - 1;
}
...
int Pop(Item aStack[], int &nTop, Item &nItem) {    // 스택에서 pop하여 nItem에 저장하는데 승패에 따라 T/F를 반환한다.
    // 받아내고 top을 감소
